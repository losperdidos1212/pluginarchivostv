# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/TURKvod/Moduls/TURKvodModul_vizor_tv.py
...
class html_parser_vizor_tv:
    ...
    def get_vizortv_categories(self, url):
        try:
            ...
            video_list_temp = []
            chan_counter = 0
            regex = re.findall('<li><a.*href="http://([^<"]*)".*>([^<"]*)</a', page)
            for text in regex:
                title = text[1]
                url = text[0]
                chan_counter += 1
                chan_tulpe = (chan_counter,
                 ...
                 None,
                 None,
                 'TURKvodModul@' + url + '@category@' + title,
                 None,
                 '',
                 '',
                 None,
    ...
    def get_vizortv_category_films(self, url):
        ...
        while True:
            ...
            page_nr = page_nr + 1
            regex_films = re.findall('<li.*<a href="http://([^<"]*)".*src="([^<"]*)".*alt="([^<"]*)"', page)
            ...
            if regex_films:
                for text in regex_films:
                    ...
                    if url_f.find('dvdgroup.php') > -1:
                        ...
                        modus = 'episodes'
                    myurl = 'TURKvodModul@' + url_f + '@' + modus + '@' + title
        ...
        self.prev_page_url = 'TURKvodModul@vizor.tv@start@Vizor.TV'
    ...
    def get_vizortv_episodes(self, url):
        ...
        chan_counter = 0
        video_list_temp = []
        regex = re.findall('<li><a.*href="([^<"]*)".*title="([^<"]*)".*src="(.*.jpg)', page)
        if len(regex) > 0:
            for text in regex:
                chan_counter += 1
                url = text[0]
